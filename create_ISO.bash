...
ALL_PVE_DEPS=$(apt-rdepends proxmox-default-kernel proxmox-ve openssh-server gnupg tasksel dmidecode qrencode btrfs-progs parted ipmitool \
  | grep -v '^ ' \
  | grep -vE '^(Reading|Build-Depends|Suggests|Recommends|Conflicts|Breaks|PreDepends|Enhances|Replaces|Provides)' \
  | sort -u)

# 把这些包本身也加进去
ALL_PVE_DEPS+=" proxmox-default-kernel proxmox-ve openssh-server gnupg tasksel dmidecode qrencode btrfs-progs parted ipmitool"
